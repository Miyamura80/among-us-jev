...
import { getRemoteGame, observeRemoteGame, stepRemoteGame, voteRemoteGame, type RemoteGame, type RemoteObservation, type RemotePlayer, type RemoteSession } from "./remote"
...
export function ServerGameView({ initialSession, onExit }: { initialSession: RemoteSession; onExit: () => void }) {
  ...
  useEffect(() => {
    const down = (event: KeyboardEvent) => { keys.current.add(event.key.toLowerCase()); if (MOVEMENT[event.key.toLowerCase()]) event.preventDefault() }
  ...
  if (game.phase === "meeting") return <MeetingView game={game} human={human} observerMode={observerMode} onVote={(targetId) => { if (human) void voteRemoteGame(session, human.id, targetId).then(update) }} />
  ...
  return <main className="game-shell"><header className="game-topbar"><button className="wordmark compact" onClick={onExit}><span className="signal-dot" /> MIRA/OS</button><div className="task-meter"><span>PRIVATE TASKS</span><div><i style={{ width: `${progress.total ? progress.done / progress.total * 100 : 0}%` }} /></div><b>{progress.done}/{progress.total}</b></div>{game.sabotage && <div className="sabotage-alert"><Warning weight="fill" /> {game.sabotage.toUpperCase()} · {Math.max(0, (game.sabotageDeadline ?? game.tick) - game.tick)}t</div>}<div className="top-actions"><span className="live-pill"><i /> SERVER</span><button onClick={onExit}><X /></button></div></header>
