...
EXCLUDE_PATH_RE='(^|/)(node_modules|__pycache__|\.venv|venv|visual-tests|e2e|tests|test|__tests__|\.git|frontend_dist|dist|build)(/|$)'
...
if [ -n "${GITHUB_STEP_SUMMARY:-}" ] && { [ "$errors" -gt 0 ] || [ "$warnings" -gt 0 ]; }; then
  {
    ...
    echo ""
    echo "**Thresholds:** warn at ${WARN_THRESHOLD} files, error at ${ERROR_THRESHOLD} files. Counts immediate \`.ts\`/\`.tsx\` children only - subfolders are the fix, not the problem."
  } >> "$GITHUB_STEP_SUMMARY"
fi
